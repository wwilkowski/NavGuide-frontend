...
const registerGuideEndpoint = "https://235.ip-51-91-9.eu/profile/guide";
...
function* signUpGoogleUser(action: types.ISignUpGoogleRequest) {
  try {
    ...
    if (status >= 200 && status <= 300) {
      ...
      showNotification(
        "success",
        i18n.t("Verification successed!"),
        i18n.t("Complete your data to finish registration process.")
      );
...
function* confirmGoogleUser(action: types.IConfirmSignUpRequest) {
  try {
    ...
    if (response.status >= 200 && response.status <= 300) {
      ...
      showNotification(
        "success",
        i18n.t("Validation successed!"),
        i18n.t("You are logged in!")
      );
      if (action.toBeGuide) {
        showNotification(
          "info",
          i18n.t("You want to be guide"),
          i18n.t("Complete your data to declare to be a guide!")
        );
